...
public class Config {
    ...
    private class ListToSizeConverter extends AbstractConverter<List<Part>, Integer> {
        @Override
        protected Integer convert(List<Part> source) {
            if (source == null) {
                return 0;
            }
            return source.size();
    ...
    public ModelMapper createModelMapper() {
        ...
        PropertyMap<Supplier, SupplierPartsDTO> supplierMap = new PropertyMap<>() {
            ...
            protected void configure() {
                using(new ListToSizeConverter()).map(source.getParts(), destination.getPartsCount());
            }
        };
